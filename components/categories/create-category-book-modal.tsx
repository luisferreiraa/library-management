...
interface CreateCategoryBookModalProps {
    open: boolean
...
export function CreateCategoryBookModal({
    open,
    onOpenChange,
    categoryId,
    categoryName,
}: CreateCategoryBookModalProps) {
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            translatorId: "",
            bookStatusId: "",
            categoryIds: [categoryId], // Pre-fill with the category ID
        },
    })

    // Initialize selectedCategories with the current category
    useEffect(() => {
        if (open && categoryId && categoryName) {
            setSelectedCategories([{ id: categoryId, name: categoryName }])
            form.setValue("categoryIds", [categoryId])
        }
    }, [open, categoryId, categoryName, form])
    ...
    const removeCategory = (categoryId: string) => {
        // Don't allow removing the main category
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[800px]">
                <DialogHeader>
                    <DialogTitle>Adicionar Novo Livro para Categoria: {categoryName}</DialogTitle>
                                                ...
                                                <div className="space-y-4">
                                                    <Popover open={commandOpen} onOpenChange={setCommandOpen}>
                                                        <PopoverTrigger asChild>
                                                            ...
                                                            >
                                                                Selecionar categorias adicionais
                                                            </Button>
                                                        </PopoverTrigger>
                                                        ...
                                                        {selectedCategories.length === 0 ? (
                                                            ...
                                                            selectedCategories.map((category) => (
                                                                <Badge key={category.id} variant="secondary" className="flex items-center gap-1">
                                                                    {category.name}
                                                                    {/* Disable remove button for the main category */}
