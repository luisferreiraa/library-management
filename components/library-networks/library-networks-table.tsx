...
import { deleteLibraryNetworksAction } from "@/app/(super-admin)/library-networks/actions"
...
export function LibraryNetworksTable() {
    const {
        paginatedEntities,
        filteredEntities,
        selectedEntityIds,
    ...
    const [selectedLibraryNetwork, setSelectedLibraryNetwork] = useState<LibraryNetwork | null>(null)
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            await deleteLibraryNetworksAction(selectedEntityIds)

            // Atualizar o estado local otimisticamente
            removeEntities(selectedEntityIds)

            const message = selectedEntityIds.length === 1
                ? "Rede de Bibliotecas excluída com sucesso"
                : `Redes de Bibliotecas excluídas com sucesso (${selectedEntityIds.length})`;
            ...
            const errorMessage = selectedEntityIds.length === 1
                ? "Erro ao excluir rede de bibliotecas"
                : "Erro ao excluir redes de bibliotecas";
    ...
    const handleEditLibraryNetwork = (entity: LibraryNetwork) => {
        setSelectedLibraryNetwork(entity)
    ...
    return (
        <div className="space-y-4">
            {hasSelection && (
                <div className="flex justify-end">
                    <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Excluir redes de bibliotecas</AlertDialogTitle>
                                <AlertDialogDescription>
                                    Tem certeza que deseja excluir {selectedEntityIds.length} rede(s)? Esta ação não pode ser desfeita.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                        ...
                        <TableRow>
                            <TableHead className="w-[50px]">
                                <IndeterminateCheckbox
                                    ...
                                    aria-label="Selecionar todas as redes"
                                />
                            </TableHead>
                        ...
                        {paginatedEntities.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={4} className="h-24 text-center">
                                    Nenhuma rede de bibliotecas encontrada.
                                </TableCell>
                            </TableRow>
                        ) : (
                            paginatedEntities.map((entity) => (
                                <TableRow key={entity.id} className={selectedEntityIds.includes(entity.id) ? "bg-muted/50" : ""}>
                                    ...
                                    <TableCell className="font-medium">
                                        ...
                                        <Link
                                            href={`/library-networks/${entity.slug}`}
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-2">
                                            <Button variant="ghost" size="icon" onClick={() => handleEditLibraryNetwork(entity)}>
                                                <Pencil className="h-4 w-4" />
                                                <span className="sr-only">Editar {entity.name}</span>
                                            </Button>
                                            <Button variant="ghost" size="icon" asChild>
                                                <Link href={`/library-networks/${entity.slug}`}>
            ...
            <Pagination
                ...
                className="mt-4"
            />
            <LibraryNetworkModal open={isEditModalOpen} onOpenChange={setIsEditModalOpen} libraryNetwork={selectedLibraryNetwork} />
        </div>
    )
}
