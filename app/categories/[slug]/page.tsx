...
import { getCategoryWithBooks } from "@/lib/categories"
import { CategoryBooks } from "@/components/categories/category-books"
...
export default async function CategoryPage({ params }: CategoryPageProps) {
    ...
    const category = await getCategoryWithBooks(slug)
                        ...
                        <div>
                            ...
                            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm">{format(new Date(category.createdAt), "dd/MM/yyyy")}</dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-muted-foreground">Última atualização</dt>
                                    <dd className="mt-1 text-sm">{format(new Date(category.updatedAt), "dd/MM/yyyy")}</dd>
                                </div>
                                <div>
                                    <dt className="text-sm font-medium text-muted-foreground">Total de livros</dt>
                                    <dd className="mt-1 text-sm">{category.books.length}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
